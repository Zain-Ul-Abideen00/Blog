...
import { Blog, blogs } from "@/../.velite/generated";
import { slug as slugify } from "github-slugger";
import Image from "next/image";
import { notFound } from "next/navigation";
import { BlogJsonLd, BlogParams } from "@/types/types";
import Comments from "@/components/Comments";
import { Suspense } from "react";

// Define TableOfContentsItem type recursively
interface TOCItem {
  url: string;
  title: string;
  items: TOCItem[];
...
// Define valid level values as a type
type TOCLevel = "two" | "three";
...
export async function generateMetadata({
  ...
  const blog = blogs.find((blog) => blog.slug === slug) as Blog | undefined;
  if (!blog) {
    return;
  }
  ...
  const ogImages: OpenGraphImage[] = imageList.map((img) => ({
